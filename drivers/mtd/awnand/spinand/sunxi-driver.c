...
inline int aw_spinand_mtd_write(unsigned int start, unsigned int sects, void *buf)
{
	...
	loff_t part_offset = spinand_sys_part_offset();
	size_t retlen;
	int ret = 0;
	...
	if (start >= part_offset)
		erase_size = info->block_size(chip);
	else
		erase_size = info->phy_block_size(chip);

	align_buf = malloc_align(info->block_size(chip), 64);
	if (!align_buf) {
...
static int spinand_mtd_erase_data(void)
{
	...
	int bootsize = CONFIG_BIMAGE_SIZE * 1024 * 1024;
	int bootblks = bootsize / phy_blk_bytes;
...
int spinand_mtd_get_flash_info(void *data, unsigned int len)
{
	...
	boot_info->logic_start_block = uboot_end + AW_SPINAND_RESERVED_PHY_BLK_FOR_SECURE_STORAGE;
...
void spinand_download_boot_param(void)
{
	struct aw_spinand *spinand = get_spinand();
	struct aw_spinand_chip *chip = spinand_to_chip(spinand);
	struct aw_spinand_info *info = chip->info;
	unsigned int erase_size = info->phy_block_size(chip);
	unsigned int boot_param_entry;
	struct sunxi_boot_param_region *boot_param = gd->boot_param;
	boot_spinand_para_t *boot_info = (boot_spinand_para_t *)boot_param->spiflash_info;

	spinand_mtd_get_flash_info(boot_info, BOOT_PARAM_SIZE);
	...
	spinand_uboot_blknum(NULL, &boot_param_entry);

	boot_param_entry += AW_SPINAND_RESERVED_PHY_BLK_FOR_SECURE_STORAGE;
	aw_spinand_mtd_write(boot_param_entry * erase_size, BOOT_PARAM_SIZE >> 9, boot_info);
}
#endif
