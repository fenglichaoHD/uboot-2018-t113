...
//uboot plat
static u32    lcd_flow_cnt[DEVICE_NUM] = {0};
static u32    lcd_open_flow_retry[DEVICE_NUM] = {0};
static s8   lcd_op_finished[DEVICE_NUM] = {0};
static struct timer_list lcd_timer[DEVICE_NUM];
static s8   lcd_op_start[DEVICE_NUM] = {0};
...
int disp_draw_colorbar(u32 disp)
{
	...
	for (i = 0; i < 4; ++i) {
		config[i].channel = de_feat_get_num_vi_chns(disp) - 1;
		config[i].layer_id = i;
		config[i].enable = 1;
...
int do_preview_init(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[]);
long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DISP_LCD_CONVERT_IS_ENABLE:
	{
		ret = bsp_disp_panel_convert_is_enabled();
		break;
	}
...
static int show_layer(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	struct disp_manager *mgr = NULL;
	struct disp_layer_config config;
	int screen_index, ret, width, height, out_w, out_h, alpha = 0;
	unsigned long addr;
	...
	screen_index = (int)simple_strtoul(argv[1], NULL, 0);
	config.channel = (int)simple_strtoul(argv[2], NULL, 0);
	config.layer_id = (int)simple_strtoul(argv[3], NULL, 0);
	config.info.fb.format = simple_strtoul(argv[4], NULL, 0);
	addr = simple_strtoul(argv[5], NULL, 0);
	width = simple_strtoul(argv[6], NULL, 0);
	height = simple_strtoul(argv[7], NULL, 0);
	out_w = width;
	out_h = height;

	if (argc > 8) {
		out_w = simple_strtoul(argv[8], NULL, 0);
		out_h = simple_strtoul(argv[9], NULL, 0);
		if (argc == 11)
			alpha = simple_strtoul(argv[10], NULL, 0);
	}

	mgr = g_disp_drv.mgr[screen_index];

	config.enable = 1;
	config.info.zorder = 64;
	config.info.mode = LAYER_MODE_BUFFER;
	config.info.alpha_mode = 1;
	config.info.alpha_value = alpha;
	...
	config.info.fb.addr[0] = addr;
	config.info.fb.addr[1] = addr + width * height;
	config.info.fb.addr[2] = addr + width * height * 5 / 4;

	config.info.screen_win.width = out_w;
	config.info.screen_win.height = out_h;
...
U_BOOT_CMD(
	show_layer,	11,	0,	show_layer,
	"show layer to display\n",
	"para1 : screen index \n"
	"para2 : channel index \n"
	"para3 : layer index\n"
	"para4 : layer format\n"
	...
	"para6 : input w\n"
	"para7 : input h\n"
	"para8 : [optional] output w\n"
	"para9 : [optional] output h\n"
);
