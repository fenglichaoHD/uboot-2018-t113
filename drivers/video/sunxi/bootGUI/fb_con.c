...
int fb_save_para(unsigned int fb_id)
{
	...
	if ((FRAMEBUFFER_NUM <= fb_id)
		|| (FB_UNLOCKED != fb->locked)) {
		printf("%s: fb_id(%d), lock=%d\n", __func__, fb_id,
			(fb_id < FRAMEBUFFER_NUM) ? fb->locked : 0xFF);
		return -1;
	}

	if (hal_save_boot_disp(fb->handle) < 0) {
		printf("%s: fb_id(%d) not open\n", __func__, fb_id);
		return -1;
	}
	...
	hal_add_kernel_iova_premap((unsigned long)fb->cv->base, fb->cv->stride * fb->cv->height);
